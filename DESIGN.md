...
objects are per-category (§10); the selection machinery shipped API-side in Loop186
(categories on the v2 stage routes) — the Restore Center UI still offers
full-restore-only, and says so.
...
| S0–S3 | devices table → journal → per-object incremental sync → selective restore — **S0 shipped Loop178**: session-row device view in `server/devices.ts` (UA grouping, no fingerprint; boundary-parsed dates), GET /api/devices in the backup-family table (identity from the session binding, cross-tenant harness pin), Settings → Vault "Manage devices" card; `keyEnvelopeStatus` stays "none" until a producer exists; CLI pair/claim/redeem now sends `user-agent: muster-cli` — **S1 shipped Loop179**: `server/sync-journal.ts` local change queue — one row per object {objectId, rev, checksum}, newest-rev-wins supersede, rev-guarded completion (a slow upload can't clobber newer work), exponential backoff + stale-claim crash recovery + dead-letter at 12, debounced idempotent drainer — **S2a shipped Loop180**: `server/sync-objects.ts` per-object envelope RIDING the v2 bundle machinery (single-file payload, authenticate-then-parse + structural single-file gates + recomputed checksum), encrypted `muster-manifest.json` (canonical fileName derivation, unique ids per object), pure rev-reconcile (upload / download / in-sync / conflict, tombstones as ordinary newer revs) — **S2b shipped Loop181**: `server/sync-pass.ts` the sync pass over injected deps — journal drain → reader must agree with its own row → pack → upload → per-push local-manifest commit → merged remote publish with the opaque guard passed through untouched; remote compare → download → verify (rev + checksum + tombstone against the entry that named the file) → apply → local commit; an unopenable remote manifest halts before anything is claimed, conflicts are reported with no side picked, and reconcile's upload side is ignored inside pull (the journal owns push) — **S2c shipped Loop183–184**: Loop183 Drive transport (stat-BEFORE-download modifiedTime guard + verify-before-write — no If-Match in Drive v3, window documented) with the base64 byte bridge, a local manifest store that fails LOUDLY on corruption, and a debounced single-claimant engine (the pass is the only journal claimant — S1's drainer deliberately not started; passphrase `null` = hold queue, the §11 flagged gate) — Loop184 the memory producer at workspace's writeMemoryFile (local-entry rev source, identical rewrites burn nothing), checksum-honest path-guarded reader with a persisted random install id, `applyMemoryFile` that never pushes back (no install ping-pong), boot wiring with restart flush + `POST /api/workspace/drive/sync` (token from `cfg.driveSync.refreshToken`) — **S3 shipped Loop186**: eight-category selective restore over the v2 restore — a path-prefix classifier pinned to the real subset scan by an exhaustiveness case, a projection that keeps only the selected files and recomputes every cross-validated field so the staging manifest still describes itself, the transcript staged only when `conversations` is selected (absent it, the live messages.db is left exactly as it was), empty / unknown / never-present selections refused before any write (`settings` and `attachments` have no bundle members, so alone they refuse rather than no-op), and a named wire contract parsing `categories` off the request body at all three v2 stage routes (file / account pull / Drive pull, 400 with the offending value named) — next B1 | K1, v2 |
