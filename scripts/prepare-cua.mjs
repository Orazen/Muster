...
async function officialBinary() {
  const cache = join(root, "node_modules", ".cache", "muster", `cua-driver-${release.version}`);
  ...
  const response = await fetch(url, { headers: { "user-agent": "Muster-packager" } });
...
await build({
  stdin: {
    ...
    resolveDir: root,
    sourcefile: "muster-cua-entry.mjs",
    loader: "js",
  },
  bundle: true,
  platform: "node",
  ...
  format: "esm",
  banner: {
    js: 'import { createRequire as __musterCreateRequire } from "node:module"; const require = __musterCreateRequire(import.meta.url);',
  },
  outfile: bundle,
  logLevel: "silent",
});
...
await writeFile(
  ...
  bundledSource.replace(
    resolverPattern,
    `${resolvers[0]}\n      if (process.env.MUSTER_CUA_SDK_LIBRARY) return resolveOverride(opts.crateName, process.env.MUSTER_CUA_SDK_LIBRARY);`,
  ),
);
