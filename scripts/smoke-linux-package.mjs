...
const executable = path.resolve(
  process.env.OMB_SMOKE_EXECUTABLE ?? path.join(root, "release", "linux-unpacked", "muster"),
);
...
mkdirSync(path.join(home, ".muster"), { recursive: true });
mkdirSync(xdgConfig, { recursive: true });
writeFileSync(
  path.join(home, ".muster", "config.json"),
...
try {
  ...
  if (health?.app !== "muster" || health.static !== true) {
    throw new Error(`unexpected embedded health response: ${JSON.stringify(health)}`);
  }
  if (!String(title).includes("Muster")) throw new Error(`unexpected renderer title: ${title}`);
