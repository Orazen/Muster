...
Muster has an Ubuntu 24.04 LTS x86_64 desktop beta. The Electron package embeds the harness server, so
installed builds do not require Node, pnpm, Swift, or a terminal at runtime.

## What works

- The native Electron window and embedded Muster server on GNOME Xorg and GNOME Wayland.
...
ARM64 are follow-ups in [issue #29](https://github.com/Orazen-Studio/Muster/issues/29).
...
```sh
git clone https://github.com/Orazen-Studio/Muster.git
cd Muster
corepack enable
...
- `release/Muster-<version>-amd64.deb`
- `release/Muster-<version>-x86_64.AppImage`
...
```sh
sudo apt install ./release/Muster-*-amd64.deb
```

Then open **Muster** from the GNOME application launcher. To remove it:

```sh
sudo apt remove muster
```

The portable AppImage does not install system files:

```sh
chmod +x release/Muster-*-x86_64.AppImage
./release/Muster-*-x86_64.AppImage
```

Application data remains local in `~/.muster`. Electron browser data and window state use the normal XDG
configuration directory (`~/.config/muster` unless the environment overrides it).
...
./release/linux-unpacked/muster
```

## Agent CLI discovery

Applications launched from GNOME do not inherit the same interactive shell `PATH` as a terminal. Muster
...
```sh
OMB_EXTRA_PATH=/your/custom/bin ./release/Muster-*-x86_64.AppImage
```

Restart Muster after installing or signing in to a CLI.

## Xorg and Wayland

The baseline shell, chat, cloud computers, and connected apps work in both GNOME session types. Muster
...
Run the CLI directly in a terminal, finish its sign-in flow, then restart Muster. If it lives outside the
...
```sh
chmod +x Muster-*-x86_64.AppImage
file Muster-*-x86_64.AppImage
```
