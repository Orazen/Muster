# MusterMobile privacy

MusterMobile is a companion for an Muster service chosen and operated by the user.
...
- Muster stores transcripts on that computer. The app does not send the developer a cloud copy.
...
Unpairing removes the connection and pairing token from the phone. Revoking the phone in Muster's Companion settings prevents that credential from reaching the computer. Transcript deletion is controlled by the Muster installation that stores it.

## Support

Questions or privacy requests can be opened at [Muster Support](https://github.com/Orazen/Muster/issues).
