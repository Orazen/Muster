...
OpenCode Go is an optional Muster engine. Muster runs the maintained
...
3. Open Muster Settings → Connections and save the key under **OpenCode
   Go API key**.

The key is stored locally as write-only configuration. Muster reports only
whether it is configured, never the value. A key saved in Muster is
...
`opencode auth login`; Muster does not edit OpenCode auth/config files.
