...
The iOS app is a thin, native client for the Muster instance running on
...
The sidecar keeps its device registry in `~/.muster/devices.json`. That is
...
The sidecar advertises `_muster._tcp` over Bonjour. The app browses with
...
Tailscale is optional. There is no Muster-operated relay or cloud copy of
...
one-time exchange for a securely stored long-lived credential. An Muster
