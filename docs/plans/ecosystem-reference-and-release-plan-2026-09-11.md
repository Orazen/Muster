...
**Loop 49:** Watch approvals now use the checked transport and captured operation; confirmation feedback follows a matching response and a final lifecycle check. Actual owned Watch acceptance passed **5/5 tests**, covering restored real UI pairing and four approval scenarios. Root **3,352 passed / 8 skipped**, Swift **270 passed**. The standalone app also now installs with the corrected Watch-only metadata. Earlier packaging and test-harness failures remain in the ledger; this does not establish a distributed release or physical haptic behavior.

**Loop 50 pending native acceptance, updated 12 September:** The five inherited edits are preserved and remain uncommitted. Fresh portable Swift has 274 passes. Astra's owned frozen snapshot now passes actual watchOS and iOS simulator builds with signing disabled (27.674s/27.414s), preserving all26 existing simulator states. Watch interaction tests for this migration remain zero. In particular, verify system-editor Done does not lose text if the chat view disappears. See the latest ownership handoff and `native-unsigned/` audit evidence; compilation is not shipping or runtime acceptance.

**Resume this correctness slice:** The original `ChatView.send()` in `ios/Watch/WatchViews.swift` cleared the reply draft before awaiting the legacy send call. Apply the existing `ComposerCoordinator` and checked `ComposerTransport` to Watch with session-owned drafts, captured destination, duplicate protection and exact acknowledgment. Preserve newer edits and navigation drafts; show failure next to the reply. Verify failed/lost responses, retry, legacy capability, account/task changes and lifecycle behavior on the owned Watch. Current draft loss must not be described as resolved by the approval tests. Then address the observed truncated roster names and canonical native mascot in slice 2.
