# Muster harness implementation plan
...
(pi, agent-orchestrator, deepseek-harness) and asking what Muster lacked by
...
**The reframe that reorders everything.** Muster is itself a harness. The
...
**On-disk forward compatibility.** `~/.muster` is shared across app versions.
