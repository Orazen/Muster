...
| Drive restore and cross-device consistency | Loop144 separate verified Drive consent, signed synthetic-provider13/13 roundtrip and guarded restore; earlier encrypted portable bundles | Fix reproduced stale-device remote overwrite with retained snapshots; real consent/fresh-device recovery/concurrent deletion; hosted account-scoped backup and actual sync remain open |
