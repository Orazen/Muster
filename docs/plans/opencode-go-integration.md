...
Add OpenCode Go as a first-class Muster engine by running the maintained
OpenCode CLI over its official Agent Client Protocol (ACP) interface and selecting
models from the `opencode-go/*` provider.

The first implementation should reuse Muster's ACP runtime rather than call
...
provide inference; Muster does not currently have a provider-independent
...
- OpenCode accepts an `OPENCODE_API_KEY` environment variable. Muster should
...
the process lifecycle Muster uses for Grok and Gemini.
...
- load that session on the next Muster turn;
...
- concurrent Muster threads without shared ACP or credential state.
...
- OpenCode Go is shown as unavailable until both its CLI and credential are
  present; Muster never selects it as a runnable default otherwise.
- The setup flow works without restarting Muster and never silently executes
  an installer.
...
6. Can the existing ACP usage metadata distinguish cumulative session totals from
   per-turn usage so Muster does not double-count it?
