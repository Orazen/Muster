...
| `yetone/cumora` (3.6k stars) | Team chat where AI agents are first-class teammates: personas, memory, claim-based coordination, seen-cursor freshness gate, big-brain triage gate, per-agent pods + BYOA daemon, one llm_calls cost ledger | The **coordination** ideas are the take: atomic claims on work units + stale-reply HOLD (agents don't trample each other) and a cheap triage model shielding the big model. Maps onto Muster's fleet/goals; the cost-ledger pattern fits Muster Cloud billing |
| `felinics/Memoh` (2.5k stars) | Every agent gets its own always-on cloud computer: filesystem, desktop, browser, network, long-term memory; bring-your-own-key; companion projects Connect-It (self-hosted connector gateway) and a Vue agent-UI design system | Validates Muster's direction (per-bot computers already exist via Box/OpenSandbox/local VM). The differentiator to copy: memory framed as a first-class per-agent surface. Muster's memory history/rollback + Drive continuum can become per-agent memory profiles |
| `oomol-lab/open-connector` (5.8k stars) | Open-source connector gateway (Pipedream/Composio alternative): 1,000+ providers, 10,000+ actions; self-hosted or hosted; SDK/CLI/MCP/HTTP surfaces; Bearer runtime token; `{success,data,meta}` envelope; `/v1/providers`, `/v1/apps/authenticated`, `/v1/connections/{service}/connect[+ /api-key]`, `/v1/actions/{id}`, `/mcp` | **Shipped as the Muster Connector**: `server/openconnector.ts` points Muster at a self-hosted OpenConnector runtime — catalog, OAuth, connection checks, action execution and agent MCP tools all run through the owner's own branding and infrastructure, ahead of Composio in backend priority |
