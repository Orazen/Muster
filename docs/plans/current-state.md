...
**Verified gates:** tsc server 0 / web 0; oxlint **0 warnings, 0 errors** on all six touched files (repo-wide 1 error remains, in a foreign stream's `src/lib/remote-client.ts` — see concurrency below); `check:electron` exit 0; new suite **9/9**; sync regression **222/222 across 10 suites** (events, journal, pass, memory, chats, wiring, objects, storage-provider, drive-transport, message-db); vitest full run (line below).
...
- **Concurrency/attribution** — a foreign stream's in-flight `src/lib/remote-client.ts` currently carries the repo's only lint error (a `typeof` narrowing at line 108); observed, reported, left where its owner put it, and excluded from this commit. **Not claimed:** deployment — push, CI, autodeploy and the GET-only prod probe follow this entry.
