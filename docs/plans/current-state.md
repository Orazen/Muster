...
### Combined batch — versioned onboarding record, OTP retry/idempotency, local-first storage seam, durable task plans, Google sign-in and Drive wiring, Mimosa deep re-run, Dokploy dashboard repair (23 September 2026, Loop192)

**Verified gates (fresh full run, all green):** `tsc` server 0 / web 0; oxlint **0 warnings, 0 errors** (978 files); vitest **369 files / 5568 passed / 8 skipped / 0 failed** (509.0s — up from the 366/5462/8/0 baseline, no decrease); `check:electron` exit 0.

- **Openbot S1 — versioned onboarding completion record** — `muster.onboarding-done:v1` `{version, completedAt, surface}` with legacy bare-`"1"` fallback (reads as v1/chat/null), first-write-wins across replays, in-place repair of malformed records, blocked storage never throws; `markOnboardingChatDone()` (flag first, then record) wired into both chat sites and wizard `finish()`; zod `safeParse` house pattern; 30/30 touched tests (20 → 30).
- **Openbot S2 + S3 — uniform retry seconds and idempotent send** — every rejection carries an integer `retryAfterSeconds ≥ 0` in body plus `retry-after`/`x-retry-after` headers (client checks/clamps/ceils; fake "wait a minute" copy removed); per-IP 8/60s send window mirroring Better Auth's rule, IP keying spent only at delegation; `Idempotency-Key` (≤128) replay map with TTL = OTP lifetime, order gates → replay → cooldown → IP; delegated relays get seconds injected; UI transport retry reuses the same key, a 429'd key is never retried, a fresh key still 429s inside cooldown; 50/50 touched tests. **Disclosed:** gate-before-replay live ordering is proven by composition (live gated-403 + deterministic policy describes), not a live static-env run.
- **Local-first Phase 1 — StorageProvider seam** — `server/storage-provider.ts` mirrors `SyncTransportDeps` byte-for-byte so a `StorageProvider` structurally satisfies the pass with no adapter and **no behavior change to `sync-pass.ts`** (comment-only); `asStorageProvider()` 1:1 delegate; `LocalStorageProvider` requires an explicit directory (empty refused, can never resolve to `~/.muster`), raw bytes (base64 bridge stays Drive-only), Drive's exact refusal strings, traversal confinement, 0700/0600; Drive method bodies moved verbatim into `googleDriveStorageProvider()`; untouched suites byte-identical (16 + 21 + 58) + 26 new = **121 evidence pass**; wiring needed **zero `server/index.ts` edits**. Narrowing disclosed: delete/list/sync/putEvent/getEvents/snapshot members deferred (not needed by P1, no primitive in the injected transport).
- **OpenMuse S4 — durable task plans** — `server/task-engine.ts`: nine-state machine (`queued/running/waiting_input/waiting_approval/paused/succeeded/failed/cancelled` + step states, kinds `checkpoint/input/approval`); two-layer legality (`NEXT` satisfies-table for targets + per-verb `requireStatus` for sources — separates `retry` from `resume` and enforces the attempt budget); illegal verb → 409 `cannot <verb> a plan in state <status>`, unknown verb → 400; lease/CAS/heartbeat/expiry with central lease-drop in `jump()`, boot parks `running → paused` (`interrupted by restart`, no auto-resume, waits/attempts survive), 5s unref'd sweep parks expired leases; every mutation in a `tx()` frame (snapshot → atomic write → SSE publish with top-level `botId`, failed write rolls memory back and emits nothing); bounds 12 steps / 8 fields / 200 plans / 200 transitions / 200 answers (resting-only prune); six additive routes anchored in the tasks zone of `server/index.ts`, each `ownsRecord`-guarded (404 `no such plan`) with actor `requestUserId`; contracts additive only; 91/91 across task-engine/steer-queue/delegations; lint 35 → 0 inside the zone via named domain types + zod boundary validation. **Deferred:** worker-facing methods unrouted, no dispatcher/goals seam, no progress-card UI, no signal-handler stop (interval unref'd).
- **Google sign-in + Drive wiring** — new `server/google-auth.ts` centralizes `googleCredentials()`, `GOOGLE_SIGNIN_SCOPES`, `isDriveScope()`, `googleDriveConnectConfigured()`; sign-in set pinned `[openid, email, profile]` with **zero Drive scopes**, Drive consent `[drive.appdata, openid]`, separation tests plus the shared `authorization()` helper enforcing the invariant across all 21 desktop-auth tests; no creds → provider list empty, sign-in 404 (no Google origin in body), Drive connect 501 `ACCOUNT_DRIVE_OFF`/`ACCOUNT_DRIVE_UNAVAILABLE` before any consent URL; creds → real consent URLs with deduped sorted scopes; LoginPage/Google button deliberately untouched (already shipped); 237/237 across 7 files. **Disclosed:** all tests fixture-based with outbound fetch/Socket blocked — a live Google round-trip remains an owner acceptance step (P2); `include_granted_scopes` dedup observation reported, `disableDefaultScope` not added (minimal diff).
- **Google console + client secret (owner-directed, assistant-executed)** — console project `muster-506214`, client `922932208725-k5vh4cl0bn9g37bkhp6vo3im3nibk78j.apps.googleusercontent.com`, redirect URIs 4 → **11** (prod calendar/workspace callback + `localhost`/`127.0.0.1:8799` dev × `api/workspace`, `api/calendar`, `api/auth` paths), persisted and reload-verified; consent in production, External, 1/100 cap. Secret transported console → local gitignored `.env` **without ever being printed**: extracted from the production container env directly to a file, validity proven by Google's token endpoint answering `invalid_grant: Malformed auth code` (not `invalid_client`); production already carried the matching `GOOGLE_CLIENT_ID` — **no VPS change made**. `.env` stays gitignored (line 42), values in no diff.
- **Mimosa deep re-run — closed, verbatim** — `scan-2026-09-23T11-15-22.181Z-31dd91398962`, depth deep, exit 0, seal `sha256:df2c0424ab29…874e6af1`; 2332 files (0 read/parse failures), functions 156126 / call edges 158006; **3747 findings — 2443 high, 1304 medium, 0 business-logic candidates**; run status **inconclusive**, completeness **partial**, **verdict effect `none`**; validated 0 / queries 0 / traces 0; suppressed test context 225; gaps: partial phase coverage and incomplete call graph (dynamic dispatch or beyond scale). Top hits are generated release bundles (Monaco language-worker JS duplicated per platform copy) and packaged better-sqlite3. **No security claim is made in either direction** — the scanner's own verdict is inconclusive and nothing was triaged; artifacts hash-sealed under `~/.mimosa/security-scans/project-22823d2eb473c7357a2b74bb/scan-2026-09-23T11-15-22.181Z-31dd91398962/`.
- **Dokploy dashboard 502 repaired (infra, read-receipts)** — `dokploy.ramagiritharun.in` served Bad Gateway: Dokploy's self-update (10:35 UTC) left the replacement swarm task stuck in `Preparing` (no image pulls, no container creates), the prior task exited 0, traefik had no healthy backend; disk (73%) and memory healthy. `docker service update --force dokploy` spawned a fresh task → container **healthy** → public probe **HTTP 200**. `muster-prod-l6w2ua` stayed Up + healthy throughout; postgres/redis/traefik untouched (11 days up). A synchronous update call blocked client-side >120s; the daemon-side update completed regardless.
- **Concurrency/attribution** — five parallel builders on disjoint zones (onboarding / email-otp / sync / tasks + contracts + index-tasks-anchor / auth zone + google files); two builders correctly reported the S4 zone's mid-edit typecheck/lint errors as foreign and preserved them rather than "fixing" another agent's file; staging is explicit-paths only — excluded set (`www/*`, `.commandcode/`, `.freebuff/`, `.zcode/`, `docs/research/glm/*`, `marketing-video/`, `www/fonts/`, `www/templates.html`) untouched. **Not claimed:** deployment — this entry's push, CI, autodeploy and GET-only prod probe follow it; deployment is claimed only once observed.

**Owner-held gates after Loop192:** Jev provider choice; Actions billing (Windows/Linux legs); Xcode Cloud auto-cancel start condition (App Store Connect page ready for owner login); App Store/TestFlight submission via `asc` gated; retention 7/7/4/6 confirmations; decision-harness script registration; real-Google round-trip acceptance (P2); hosted storage-gate decision 14 (`storageGateFor` unchanged). **Closed this loop:** Mimosa re-run (report sealed and reported; its inconclusive verdict keeps the no-security-claims rule in force) and Google OAuth credentials (console URIs + local env; values never in the repo).

### Conversation sync (local-first P4) — per-thread objects, tombstones, one routing seam (23 September 2026, Loop197)

**Verified gates (fresh full run, all green):** `tsc` server 0 / web 0; oxlint **0 warnings, 0 errors** (983 files); vitest **371 files / 5612 passed / 8 skipped / 0 failed** (601.4s — up from the 369/5562/8/0 baseline, no decrease); `check:electron` exit 0; P4-focused regression **359 passed across 15 files** (all eight sync suites + branching, seed-answer, delegations, steer-queue, restore-apply, bundle-restore-v2).

- **P4 shipped — conversations sync as per-thread objects** — `server/sync-chats.ts`: object id `chat:<threadId>`, canonical payload `{schema, threadId, activeLeafId, messages}` (fixed key order, row order preserved, every message re-serialized from its stored `json` so cards/activity/compaction/privacy counts ride through byte-identical). The producer takes rev from the local manifest and checksum from the CURRENT thread bytes, skips byte-identical rewrites, and refuses thread ids the envelope cannot carry without failing the write that already succeeded. 14/14 new tests.
- **A delete is data** — `deleteThread` publishes the canonical tombstone object (`payload: ""`, its sha256) at the next rev, and the applier installs a peer's tombstone with `deleteThreadSilently` (no notification), so a pull deletes the thread instead of resurrecting it and the two installs never ping-pong the deletion.
- **One seam, two producers** — new `server/sync-dispatch.ts` routes the pass's `readObject`/`applyObject` by `objectType` (chat → chat side, everything else → memory, whose object-id guard refuses foreign types by name). `sync-pass.ts` itself is **untouched** — still the S2b engine its 16 tests pin.
- **P4 gate mapping** — *round-trip identity*: A → destroy → B is asserted as **byte equality** of the payload (ids, roles, kinds, text, tool tail, branch head); *incremental-bytes* is structural and measured — appending to thread B leaves thread A's rev, checksum and payload byte-identical; *full suite* green at 371/5612/8/0.
- **message-db producer wiring** — the chat hook fires **after commit, never inside a transaction** (append / persistMessagePath / updateMessage / standalone leaf move), with quiet internals so the transactional callers announce once; `readThreadRows` is DB-only (never a legacy JSON import, which would change the payload between two reads); `replaceThreadFromSync` installs rows-then-leaf in ONE transaction with no notification.
- **Disclosed deferrals** — P5 (typed event receipts, stable `device_id`, key envelopes, device revocation) is untouched: it is the plan's next phase with its own multi-device gate. `commitSeedAnswer` folds into the next append's rev — content is never stale (the payload is serialized live at push time), only rev granularity waits for a later trigger. Attachments ride inside the message objects exactly as they ride in the v2 bundle; no separate attachment object type was invented.
- **Concurrency/attribution** — this commit stages **explicit paths only**: the boot-wiring hunks in `server/index.ts` were staged as a filtered patch because a foreign stream's `taskThread` routine hunk shares the file (left unstaged for its owner). Desktop (`src/*`) and iOS/Watch (`ios/*`, `companion/*`) surface work for the same feature is in flight in parallel and lands in its own commit; an unrelated routines stream (`server/routines*`, `RoutinesPage`, `src/lib/routines.ts`) stays entirely unstaged here. **Not claimed:** deployment — push, CI, autodeploy and the GET-only prod probe follow this entry; deployment is claimed only once observed.

### Local-first surfaces — desktop settings section, iPhone + Watch read-only status (23 September 2026, Loop198)

**Verified gates:** `tsc` web 0; check:electron exit 0; vitest **372 files / 5619 passed / 8 skipped / 0 failed** (500.1s — up from the 371/5612/8/0 baseline, no decrease); companion `node --check` exit 0 on both files; oxlint on companion files 0/0; desktop slice 7/7; **`swift test` 463 passed / 0 failed** (457 baseline +6, no decrease); `swiftc -parse` exit 0 across all seven changed Swift files; **XcodeGen + `xcodebuild -destination 'generic/platform=iOS Simulator' CODE_SIGNING_ALLOWED=NO` → BUILD SUCCEEDED** (iOS app + embedded Watch app + complication + widget + CompanionCore). **Foreign-stream note:** the global `tsc`/oxlint pair currently reports 1+1 errors, both inside `server/routines.test.ts` (the in-flight routines stream's own file) — untouched and unstaged; every file in this slice is clean in both gates.

- **Desktop (browser build untouched)** — a read-only **Local-first** section inside the existing Settings surface (no new route, no layout-shell change, hidden outside the Electron build): `src/lib/local-first-status.ts` validates only `GET /api/workspace/google/status` (`workspaceBackupAvailable`, `installationDrive.operationsAvailable`, `accountDrive.available/connected`) and `GET /api/config` (`storageGate.required/satisfied`); `LocalFirstSection` renders local-by-default, Drive configured/connected/absent, and the storage-gate state, and states plainly that these endpoints report no last-sync time or per-conversation progress so the UI never implies an active transfer. **No `localStorage`/`sessionStorage` keys added, no new IPC, no server file touched, no path or token rendered.** 7/7 tests (endpoint contract + render).
- **iPhone — Settings → Computer → Workspace data** — adopted and rewrote the interrupted builder's orphan into a **bounded, memory-only status projection**: provider-neutral `StorageDestination` (the ungrounded `telegram` case removed — no capability field can report it), destination inference only from explicit usable flags, connected account Drive taking precedence over the installation backup flag, a boolean "snapshot status unavailable" instead of a raw error, and **verified timestamps kept distinct from successful ones** (the orphan could present a last-success as a completed verification). No fields for paths, provider ids/names, tokens, passphrases, errors or history.
- **Watch — Settings → Connected to → Workspace data** — the Watch fetches from its **directly paired** computer instead of trusting phone-shared state; reports are memory-only and cleared when pairing/connection changes. Both surfaces describe only the pages the device has loaded (`3 bots · 2 rooms · 48 messages` shaped as *loaded*, not owned).
- **Companion route boundary** — exactly two read-only GETs allowed (`/api/workspace/google/status`, `/api/workspace/snapshots/policy`); boundary tests for full **and** approvals-only devices prove snapshot policy `POST`, snapshot `POST /run`, passphrase `POST`/`DELETE`, workspace export, Drive push and capability mutations all stay denied.
- **Frozen** — bundle identifiers, Watch/widget identifiers, entitlements, App Groups, `ios/project.yml`, Info plists, version/build numbers, dependencies, saved choices/drafts/pairing/sessions, navigation shells and mascot layout: untouched.
- **Disclosed** — there is **no companion-facing conversation-sync receipt on the wire yet** (the P4 engine reports through the pass, not an endpoint), so the phone and watch deliberately do not claim conversation sync; a bounded `GET /api/workspace/companion/status` shape (storage destination, backup attempt/success/verified, conversation-sync last push/pull) is recorded as the next server-side slice rather than guessed at from the client.
- **Concurrency/attribution** — a foreign routines stream holds `server/routines*`, `src/lib/routines.ts`, `RoutinesPage` and the `taskThread` hunk in `server/index.ts`; its `server/routines.test.ts` currently carries **1 tsc error (TS6133 unused `botId`) and 1 lint error (assertion without SAFETY comment)** — observed, reported, left exactly where its owner put it, and excluded from this commit. Staging is explicit-paths only; the excluded parallel set stays unstaged. **Not claimed:** deployment — push, CI, autodeploy and the GET-only prod probe follow this entry.

### Companion receipt — the phone stops guessing, the computer answers (24 September 2026, Loop199)

**Verified gates:** `swift test` **465 passed / 0 failed** (463 baseline +2, no decrease); `swiftc -parse` exit 0 on all changed Swift; **XcodeGen + `xcodebuild -destination 'generic/platform=iOS Simulator' CODE_SIGNING_ALLOWED=NO` → BUILD SUCCEEDED** (app + embedded Watch + complication + widget); tsc server 0 / web 0; oxlint **0 warnings, 0 errors** (988 files); `check:electron` exit 0; receipt mapping **12/12**; companion-boundary + workspace-backup family regression **160/160**; vitest **373 files / 5634 passed / 8 skipped / 0 failed** (543.0s — up from the 372/5619/8/0 baseline, no decrease).

- **One read-only receipt: `GET /api/workspace/companion/status`** — `server/companion-status.ts` is a pure function over state the server already keeps: `storageDestination` (the same capability booleans the existing `/api/workspace/google/status` advertisement uses, extracted into one shared `workspaceBackupCapability()` helper so the two documents **cannot drift**), `backup` (last attempt / last success / run outcome / **last verified kept apart** — a successful upload and a completed download-and-verify round trip are different facts), and `conversationSync` (the per-account push/pull stamps `sync-state.ts` writes after a transport confirms — the first wire conversation sync (P4) can be read from).
- **The document cannot carry anything sensitive** — a test pins the exact key set: no path, provider id or name, token, passphrase state, raw error, policy or history. "Never happened" is `0` / `"unknown"`, never an epoch; nonsense stamps (negative, NaN) are refused rather than echoed; conversation sync says `success` only when a transport stamped one and **never** `failed`, because nothing records failures (disclosed, not papered over).
- **Placement** — the route sits above the installation wall beside the capability advertisement: session-scoped, reports only the signed-in account's own stamps, `no-store`, and moves nothing. The companion allowlist gains exactly this one GET; snapshot policy `POST`, snapshot run, passphrase writes, export, Drive push and capability mutations stay denied to full **and** approvals-only devices.
- **iPhone + Watch** — `CompanionReceipt` decodes the new document with every field optional; an **unrecognized future destination decodes to nil** and the UI falls back to the capability-derived answer rather than guessing. The receipt is fetched **before** the availability guard, so a hosted computer still reports when its user's Drive last carried a conversation. A new **Conversations** row shows pushed/pulled timestamps, "Last attempt failed", "Not synced yet", or **"Not reported"** when the computer has no receipt (an older computer) — never a fabricated success. The computer's own word wins for the destination row; the watch still reads from the computer it is directly paired to.
- **Tests** — 12 receipt-mapping tests (destination precedence, available≠connected, unavailable-not-a-guess, verified≠success, pass-through of failed/skipped, stamp refusal, no-stamps-is-unknown, push-only/pull-only, exact key set, provider name absent) and 2 new Swift tests (receipt precedence + future-destination fallback, conversation-sync reported/not-reported incl. zero/NaN), with the three existing client tests updated for the third read-only request and the hosted path proving the receipt is read before the guard.
- **Not claimed:** deployment — push, CI, autodeploy and the GET-only prod probe follow this entry; the receipt's live shape is additionally verifiable only by an owner with a paired device.

### P5a — typed event receipts, device identity, and the journal bug that made every second push dead-letter (24 September 2026, Loop200)

**Verified gates:** tsc server 0 / web 0; oxlint **0 warnings, 0 errors** on all six touched files (a foreign stream's `src/lib/remote-client.ts` carried the repo's only lint error during this loop — reported, untouched, unstaged); `check:electron` exit 0; new suite **9/9**; sync regression **222/222 across 10 suites** (events, journal, pass, memory, chats, wiring, objects, storage-provider, drive-transport, message-db); vitest **375 files / 5673 passed / 8 skipped / 0 failed** (1134.5s, exit 0 — up from the 373/5634/8/0 baseline, no decrease; wall-clock inflated by parallel streams on the same machine, the 9-minute quiet-machine baseline is 373 files).

- **A journal bug reproduced, then fixed** — `sync_journal`'s supersede `UPDATE` binds `objectType = ?` but was passed `input.objectId`, so **the second and every later write of any object overwrote its type with its id** (verified empirically before the fix: `{"objectType":"chat:thread-a"}`). P4's dispatcher keys on `objectType`, so those pushes were routed to the wrong producer, refused, retried, and dead-lettered after 12 attempts — in practice **only rev 1 of any object ever reached the remote**. One argument, silent for every object after its first rev. Fixed, with a regression test that writes twice and pushes the superseded rev through the real dispatcher (`pushed: chat:thread-a@2`, zero errors).
- **Typed event receipts (R5)** — `server/sync-events.ts` is the receipt VIEW of the journal, never a second queue: every event is written at a point the journal already passes (a row really enqueued, an object really applied) and carries exactly `{v, type, objectId, objectType, rev, deviceId, at}`. Vocabulary derived, not closed: rev 1 is `created`, later revs `updated`, a producer that knows it is deleting says `deleted` (the chat producer now passes the tombstone flag), an install that replayed a peer's rev says `applied` (recorded in the dispatch layer, so `sync-pass.ts` stays the engine its tests pin, and a **rejected** apply earns nothing). A duplicate or stale enqueue is not a change and earns no event.
- **No event can carry content** — a test writes a real transcript message containing a password, an email and a token-shaped string, then greps the whole on-disk stream for all three and pins the exact key set. The receipt says what changed and which device said so; the bytes live in the encrypted object, not the log. A receipt that cannot be written (disk on fire) never fails the write, push or apply it describes; a damaged line is skipped rather than blinding the read.
- **Device identity (R12, P5's first clause)** — `syncInstallId` moved into `sync-events.ts` as the identity home (P5 makes it the device_id) and is re-exported from `sync-memory.ts`, so every existing importer and test is untouched: a random uuid persisted at mode 0600, regenerated rather than trusted, never derived from hardware. It is the same value the sync objects already carry, so the receipt stream and the object stream cannot disagree about who produced a rev. Stable within an install, different across installs — both pinned.
- **The P5 gate clause, run for real** — two installs, one owner: A writes a message and sees `chat.created` under **A's** device id; A's pass pushes the encrypted object and the manifest; B's pass pulls, verifies and applies, ends up holding the same message and branch head, records `chat.applied` under **B's own** id for A's object id and rev, and enqueues **nothing** (no ping-pong). A's local manifest and B's are separate files — sharing one would make B believe it had already applied A's object, which is a reconciliation truth rather than a test convenience.
- **Bounded retention, honestly labelled** — one `sync-YYYY-MM-DD.ndjson` per UTC day inside the events directory (the `sync-` prefix keeps these apart from the unrelated thread runtime evidence already there), at most `EVENT_MAX_FILES` (30) and `EVENT_MAX_BYTES` (8 MB), oldest pruned first, only files this module wrote. **Generational compaction is NOT claimed**: that needs the generation counter P6 introduces, and the caps are documented as the bound they are.
- **P5 remainder, untouched and disclosed** — key envelopes (so `keyEnvelopeStatus` can stop being `"none"`) and device/OAuth-token revocation with `muster sessions --revoke` semantics are the remaining P5 clauses; each needs its own gate and lands as its own loop rather than smuggled in here.
- **A test race found while gating this loop, and its honest correction** — the first full run of this slice failed 4 files / 20 tests, every one of them a subprocess-spawning harness whose fixture "did not start" or timed out; the run took 32 minutes against a 9-minute baseline. Re-running the files in isolation left exactly one red: `workspace-auth-harness.test.ts` asserting **no outbound network calls** while its own fixture config carries a Telegram bot token. The culprit, named from the harness's blocked-attempt log, is `https://api.telegram.org/bot…/getUpdates` from `startTelegramChatLoop()` (server/index.ts) — **pre-existing code this slice never touched**, which polls every 3 seconds whenever a token is configured. So the failure was a wall-clock race, not a regression: at 26s the fixtures exited before the first tick and the suite passed; at 116-213s they lived past it and an unsolicited poll landed in the assertion. An intermediate conclusion of mine ("my change caused it", inferred from green-at-`725c7d1` vs red-at-`c080410`) was **wrong and is corrected here**: those two runs differed in machine load, not in behavior, and the server boots and serves health normally under the change. The fix is isolation, not behaviour: the harness now sets `TELEGRAM_CHANNEL_BOT_ID=""`, which switches off only the unsolicited chat poll (the backup transport under test is untouched), so "no outbound" means the same thing on a fast machine and a slow one. 139/139 in 65s afterwards.
- **Concurrency/attribution** — a foreign stream's in-flight `src/lib/remote-client.ts` carried the repo's only lint error during this loop (reported, untouched, unstaged), and the same stream's `git add -A` committed this slice's six files as `c080410` with an accurate message while the ledger was still being written. **Not claimed:** deployment — push, CI, autodeploy and the GET-only prod probe follow this entry.

### Two macOS failures fixed — the updater's 15-second panic, and a privacy grant that had to go to a different app (24 September 2026, Loop201)

**Verified gates:** tsc web 0; oxlint **0 warnings, 0 errors** across all eight touched files; `check:electron` exit 0; new suites **9 passed** (5 node + 4 vitest) with the existing CUA and desktop suites green (**53 passed across 4 vitest files**, existing `cua-runtime` suite included); full vitest follows this entry.

- **The updater told you a lie about your own build** — "This can happen on an unsigned build" is what the stuck-install card said, and `Restart didn't finish` appeared after **15 seconds**. The installed app is Muster **1.18.0, Developer ID signed** (`Developer ID Application: THARUN RAMAGIRI / 7375K23WFU`) with the `trusted-mac-updates` marker present — so on this build the in-app path is *supposed* to work, and a signed ~180 MB apply genuinely takes longer than 15 seconds. The banner declared a healthy install hung, told you the cause was a signature you do not have, and offered a download instead of waiting. Now: two honest thresholds (`INSTALL_STILL_WORKING_MS` 45s → "Still applying the update… macOS is verifying and swapping the new app", `INSTALL_STUCK_MS` 120s → "Restart didn't finish"), the unsigned wording reserved for the builds that actually lack the marker, and a stuck card that says what happened rather than blaming a signature. The timer is a pure exported function, tested at 15s/45s/120s.
- **"I granted it and it still says no" — the grant was going to the wrong app** — root cause, with the daemon's own evidence: the machine was running `cua-driver serve --cua-internal-gate-engaged --cua-internal-gate-missing-screen-recording`, and the driver's status answered `screen_recording: false` for **`com.trycua.driver` (TeamID YCK386LBJ7, the CUA vendor)** while accessibility was true. macOS keys privacy per **binary**, and the screenshots are taken by CuaDriver — a separately signed app — not by Muster (`com.muster.app`, TeamID 7375K23WFU). Granting Muster, which the old copy invited, changes nothing for the driver. Worse, both of Muster's own checks (the SDK preflight and the fresh in-process capture) measure *Muster's* identity, so the app could report "granted" while the driver stayed gated — which is exactly the false negative you hit. The embedded path now asks the **host** for its own status first (`hostPermissionStatus`, fed by the driver's own `permissions status --json`, trusting it only when it reports a foreign bundle id with real booleans) and refuses with a message that names the app, the id and the pane: *Screen Recording must be granted to CuaDriver (com.trycua.driver) in System Settings → Privacy & Security … granting Muster does not grant the driver that takes the screenshot*.
- **The repair is one click now** — `localComputerRepair()` (pure, tested) turns a blocked local session into words plus the exact panes: the computer panel renders **"Open Screen Recording settings"** and **"Open Accessibility settings"** through the existing `perm:open-settings` bridge, names CuaDriver (not just Muster) in the copy, and offers **no** privacy buttons when the real problem is "computer access is off" — a settings pane cannot fix a toggle.
- **Local VM isolation: already in Muster, and ahead of the comparison** — `LocalVmIsolationMode = "shared" | "perBot"`, `localVm: { mode, maxInstances }`, the global per-bot cap, the enforced message "unused desktops recycle automatically after 8 idle hours" (server/index.ts) and the settings screen are all present in this tree; the OpenMausBot checkout has no VM isolation knobs at all (its `perBot` matches are unrelated files). What the screen cannot do today is start, because the driver refuses to serve while its Screen Recording gate is engaged — the same grant this loop now tells you where to give. **Not claimed:** a live grant or a live VM boot; both need the one-time System Settings action on your machine.
- **Not claimed:** deployment — push, CI, autodeploy and the GET-only prod probe follow this entry.
