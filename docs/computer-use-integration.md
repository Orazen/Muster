# Computer use & browser use in Muster

Decision doc, 2026-08-12. How bots in Muster get local computer use and
...
│     one TCC prompt, named Muster          │ unix socket (private)
...
1. **Spawn from the Electron main process, never from the server/gateway
   ...
   Spawned from Electron main → the grant is Muster's, users see ONE
   prompt named Muster, and the bundled driver inherits it. Spawned from
...
- Ship the binary at `Muster.app/Contents/Resources/cua-driver`,
