...
| File | Purpose |
|---|---|
| `Muster-<version>-setup.exe` | the installer |
| `latest.yml` | **the update feed** — see step 4 |
| `Muster-<version>-setup.exe.blockmap` | differential updates |
| `Muster-<version>-x64.zip` | portable, not used by the updater |
...
2. The chat window renders (not the error page). Server logs land in
   `%APPDATA%\Muster\logs\server.log`.
...
- **`Muster-<version>-setup.exe`** is what `latest.yml` references by name and
  sha512. The auto-updater downloads exactly this.
- **`Muster-setup.exe`** is a byte-identical copy that gives the README's
  `/releases/latest/download/Muster-setup.exe` button a stable URL. This
  mirrors `Muster.dmg` sitting beside `Muster-<version>.dmg`.
