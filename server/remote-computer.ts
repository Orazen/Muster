...
// inside the VM and Muster never exposes another inbound port.
...
export const REMOTE_CUA_SESSION = "muster";
export const REMOTE_CDP_HELPER = "/opt/ogb/muster-cdp.mjs";
...
export function remoteComputerBootstrapCommand(botName: string): string {
  ...
  return [
    ...
    `tmux has-session -t work 2>/dev/null || tmux new-session -d -s work 'echo; echo "  ▦ ${safeName}'"'"'s computer — Muster"; echo; exec bash -i'`,
    "echo bootstrapped",
  ].join("\n");
}
