...
export function createControlClient(options?: {
  ...
  const url = options?.url ?? process.env.MUSTER_CONTROL_URL ?? "";
  const token = options?.token ?? process.env.MUSTER_CONTROL_TOKEN ?? "";
  ...
  return {
    ...
    async requestHelp(reason: string): Promise<string | null> {
      ...
      try {
        ...
        // SAFETY: body is untyped JSON off the wire — parse the one field
        // this client reads instead of narrowing with typeof.
