...
async function startTurn(
  ...
  opts?: {
     ...
     * of merely mounting that VM's computer tools on the AGENT's provider. */
    runOn?: RoutineRunOn;
  ...
  void (async () => {
    ...
    try {
      ...
      if (wants === "vm") {
        ...
        if (!cua) throw new Error("CUA Driver is not ready for this computer — check permissions and restart Muster");
...
try {
  ...
  console.log(`muster webhook receiver on ${webhookIngress.baseUrl}`);
} catch (error) {
...
async function runGroupMemberTurn(
  ...
  const roster = group.memberIds
    ...
    .join(", ");
  const system = [
    `You are ${bot.name}, a bot in the room "${group.name}" in Muster.`,
    bot.title && `Role: ${bot.title}.`,
    bot.description && `About: ${bot.description}`,
...
(async () => {
  ...
  try {
    ...
    // Drive connect/callback and the v2 bundles live behind an ordered route
    // table there. This registration sits exactly where the inline family
