// Drive recovery consent is independent of Google login and Calendar grants.
...
const grantSchema = z.object({
  ...
  scopes: z.array(z.string().min(1).max(2048)).min(1).max(100)
    .refine(scopes => scopes.includes(DRIVE_APPDATA_SCOPE)),
});
export type DriveGrant = z.infer<typeof grantSchema>;
export type DriveStateBinding = z.infer<typeof bindingSchema>;
export type DrivePendingState = z.infer<typeof stateRowSchema>;
export type DriveGrantInput = Omit<DriveGrant, "refreshToken" | "generation"> & { refreshToken?: string | null; expectedGeneration: number };

function initialize(db: DatabaseSync): void {
  db.exec(`CREATE TABLE IF NOT EXISTS drive_oauth_states (
  ...
  CREATE INDEX IF NOT EXISTS drive_oauth_states_user ON drive_oauth_states(userId);
  ...
  CREATE TABLE IF NOT EXISTS drive_grant_generations (
...
export function createDriveState(db: DatabaseSync, binding: DriveStateBinding, now = Date.now()): DrivePendingState & { state: string; codeChallenge: string } {
  if (!bindingSchema.safeParse(binding).success || !timestamp.safeParse(now + STATE_TTL_MS).success) {
    throw new Error("Invalid Drive consent binding");
  }
  initialize(db);
  db.prepare("DELETE FROM drive_oauth_states WHERE expiresAt <= ?").run(now);
  const generation = advanceGeneration(db, binding.userId);
  db.prepare("DELETE FROM drive_oauth_states WHERE userId = ?").run(binding.userId);
  const state = randomBytes(32).toString("base64url");
  const codeVerifier = randomBytes(32).toString("base64url");
  const nonce = randomBytes(32).toString("base64url");
  const expiresAt = now + STATE_TTL_MS;
  db.prepare(`INSERT INTO drive_oauth_states
...
export function consumeDriveState(db: DatabaseSync, input: DriveStateBinding & { state: string }, now = Date.now()): DrivePendingState | null {
  if (!bindingSchema.safeParse(input).success || !/^[A-Za-z0-9_-]{43}$/.test(input.state) || !timestamp.safeParse(now).success) return null;
  initialize(db);
  const row = db.prepare(`DELETE FROM drive_oauth_states
...
export function getDriveGrant(db: DatabaseSync, userId: string): DriveGrant | null {
  initialize(db);
  const row = db.prepare("SELECT userId, googleSub, accessToken, refreshToken, expiresAt, scopes, generation FROM drive_grants WHERE userId = ?").get(userId);
  if (!row) return null;
...
export function isDriveGrantCurrent(db: DatabaseSync, grant: DriveGrant): boolean {
  const saved = getDriveGrant(db, grant.userId);
  const epoch = db.prepare("SELECT generation FROM drive_grant_generations WHERE userId = ?").get(grant.userId);
...
export function saveDriveGrant(db: DatabaseSync, input: DriveGrantInput): DriveGrant {
  initialize(db);
  const current = db.prepare("SELECT generation FROM drive_grant_generations WHERE userId = ?").get(input.userId);
  if (!Number.isSafeInteger(input.expectedGeneration) || current?.generation !== input.expectedGeneration) {
    throw new Error("Drive consent was superseded or disconnected");
  }
  const prior = getDriveGrant(db, input.userId);
  // Read the identity even if an existing token row is corrupt; never reuse or replace across subjects.
  const identity = db.prepare("SELECT googleSub FROM drive_grants WHERE userId = ?").get(input.userId);
  if (identity && identity.googleSub !== input.googleSub) throw new Error("Drive account mismatch");
  ...
  if (!parsed.success) throw new Error("Drive grant requires appdata scope and valid offline tokens");
  const grant = parsed.data;
  const saved = db.prepare(`INSERT INTO drive_grants (userId, googleSub, accessToken, refreshToken, expiresAt, scopes, generation)
    SELECT ?, ?, ?, ?, ?, ?, ? WHERE EXISTS (
      SELECT 1 FROM drive_grant_generations WHERE userId = ? AND generation = ?
  ...
  if (!saved) throw new Error("Drive consent was superseded or disconnected");
  return grant;
}

/** Local removal only: never revokes Google's shared login/Drive authorization. */
export function disconnectDrive(db: DatabaseSync, userId: string): void {
  initialize(db);
  advanceGeneration(db, userId);
  db.prepare("DELETE FROM drive_grants WHERE userId = ?").run(userId);
  db.prepare("DELETE FROM drive_oauth_states WHERE userId = ?").run(userId);
}

function advanceGeneration(db: DatabaseSync, userId: string): number {
  const row = db.prepare(`INSERT INTO drive_grant_generations (userId, generation) VALUES (?, 1)
