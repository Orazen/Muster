// OpenAI driver — chat-completions API with SSE streaming. Same shape as
// grok.ts (xAI is OpenAI-compatible): transcript-replay, token-level
// content.delta events, generateText for titles/thread names.
import type {
  DriverCreateInput,
  ProviderDriver,
  ProviderInstance,
...
export const OpenAIDriver: ProviderDriver<OpenAIConfig> = {
  ...
  async create(input: DriverCreateInput<OpenAIConfig>): Promise<ProviderInstance> {
    ...
    const sendTurn = async (turn: SendTurnInput) => {
      ...
      appendNative(threadId, { dir: "out", source: "openai.chat.completions", msg: { model: turn.model, messages } });
    ...
    const snapshot = async (): Promise<ProviderSnapshot> => {
      if (!apiKey) {
        return {
          state: "unavailable",
          reason: `no OpenAI API key — add it in Settings → Providers, or set ${config.apiKeyEnv}`,
        };
      }
    ...
    return {
      ...
      generateText: async (prompt: string) => {
        const { text } = await complete([{ role: "user", content: prompt }], "gpt-4o-mini", { stream: false });
        return text;
      },
      dispose: async () => {
        for (const { abort } of active.values()) abort.abort();
