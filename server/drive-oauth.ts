...
const FAILURE = 'Google Drive authorization failed';
...
export class DriveOAuthRefreshError extends Error {
  readonly reconnectRequired: boolean;
  constructor(reconnectRequired = false) {
    super('Google Drive refresh failed');
    this.reconnectRequired = reconnectRequired;
  }
}

export class GoogleDriveOAuthProvider {
  private readonly options: GoogleDriveOAuthOptions;
  private readonly request: typeof globalThis.fetch;
  private readonly verifyIdToken: GoogleIdTokenVerifier;

  constructor(options: GoogleDriveOAuthOptions) {
    try {
      ...
      throw new Error('Invalid Google Drive OAuth configuration');
    }
  ...
  authorizationUrl({ state, nonce, codeChallenge }: { state: string; nonce: string; codeChallenge: string }): string {
    ...
    url.search = new URLSearchParams({
      ...
      response_type: 'code',
      scope: `openid ${GOOGLE_DRIVE_APPDATA_SCOPE}`,
      access_type: 'offline',
      prompt: 'consent',
      state,
      nonce,
  ...
  async refresh(grant: GoogleDriveOAuthGrant): Promise<GoogleDriveOAuthGrant> {
    try {
      if (!z.object({ googleSub: nonemptyString, accessToken: nonemptyString, refreshToken: nonemptyString,
        ...
        scopes: z.array(nonemptyString).refine(scopes => scopes.includes(GOOGLE_DRIVE_APPDATA_SCOPE)),
      }).safeParse(grant).success) throw new DriveOAuthRefreshError(true);
      ...
      if (!response.ok) {
        ...
        throw new DriveOAuthRefreshError(failure.success && failure.data.error === 'invalid_grant');
      }
      ...
      if (!scopes.includes(GOOGLE_DRIVE_APPDATA_SCOPE)) throw new DriveOAuthRefreshError(true);
      const now = Date.now();
      const expiresAt = now + token.expires_in * 1000;
      if (!Number.isSafeInteger(expiresAt) || expiresAt <= now || expiresAt > 8.64e15) throw new DriveOAuthRefreshError();
      ...
      throw new DriveOAuthRefreshError(error instanceof DriveOAuthRefreshError && error.reconnectRequired);
    }
  }

  async exchange(code: string, { codeVerifier, nonce }: { codeVerifier: string; nonce: string }): Promise<GoogleDriveOAuthGrant> {
    try {
      ...
      if (!scopes.includes(GOOGLE_DRIVE_APPDATA_SCOPE)) throw new Error(FAILURE);
      const now = Date.now();
      const expiresAt = now + token.expires_in * 1000;
      ...
      const grant: GoogleDriveOAuthGrant = {
        googleSub,
