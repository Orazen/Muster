...
async function handle(message: Json): Promise<void> {
  ...
  if (method === "tools/call") {
    ...
    if (slugs.length) {
      ...
      send(textResult(
        id,
        `Muster showed the user a secure connection card for ${slugs.join(", ")}. End this turn now. The app will continue the task automatically after the connection finishes.`,
      ));
      return;
    }
