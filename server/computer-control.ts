...
export class ComputerControl {
  ...
  requestHelp(botId: string, reason: string): ControlSnapshot {
   ...
   * `reason` arrives pre-trimmed/capped from the route's I/O boundary. */
  requestHelpLease(botId: string, reason: string) {
    ...
    if (entry.helpReason === null) {
      ...
      const text = reason.trim().slice(0, MAX_REASON_CHARS);
  ...
  expireHelp(botId: string, requestId: string): ControlSnapshot {
    const entry = this.entries.get(botId);
