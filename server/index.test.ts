...
beforeAll(async () => {
  ...
  writeFileSync(join(staticDir, "index.html"), "<!doctype html><title>Packaged Muster</title>");
...
describe("harness HTTP API", () => {
  ...
  it("serves packaged UI assets and preserves API 404s", async () => {
    ...
    expect(await spa.text()).toContain("Packaged Muster");
  ...
  it("validates a Composio project key, creates a Session, and keeps externally stored secrets off disk", async () => {
    ...
    const disk = JSON.parse(readFileSync(join(home, ".muster", "config.json"), "utf8"));
  ...
  it.skipIf(process.platform === "win32")("stores the credentials file with owner-only permissions", () => {
    expect(statSync(join(home, ".muster", "config.json")).mode & 0o777).toBe(0o600);
  });
  ...
  it("creates an independent webhook, accepts a delivery, deduplicates it, and rotates its secret", async () => {
    ...
    if (process.platform !== "win32") {
      expect(statSync(join(home, ".muster", "webhooks.json")).mode & 0o777).toBe(0o600);
    }
  });
...
describe("bot memory API", () => {
  ...
  const workspaceOf = (botId: string) => join(home, ".muster", "workspaces", botId);
  ...
  it("refuses every coat of path traversal without reading the target", async () => {
    ...
    try {
      ...
      writeFileSync(join(home, ".muster", "secret.md"), "TOP-SECRET-MARKER sibling");
