...
// QuickRepliesEditor (https://github.com/milind-soni/OpenMausBot,
// Apache-2.0 — full license text and attribution in
// public/third-party-notices.txt, shipped with the app); the storage key
// moved into `QuickReply.storageKey` so this screen and the composer can
// never drift apart.
import SwiftUI
import CompanionCore

struct QuickRepliesEditor: View {
    @AppStorage(QuickReply.storageKey) private var stored = ""
        ...
        List {
            Section {
                ForEach(replies) { reply in
                    ...
                    .buttonStyle(.plain)
                    .accessibilityIdentifier("quick-reply-row-\(reply.id)")
                }
                .onDelete { offsets in
                    var next = replies
