...
## Watch approval acceptance

Loop 49 verified the actual standalone Watch app on an owned Series 9 (41 mm), watchOS 26.5 simulator: **5/5 tests passed**, with 47 helper assertions. The cases cover restoration of pairing completed through the real Watch UI, grant-before-answer ordering, failed grant and explicit retry, unavailable answer after a saved preference, and duplicate taps plus leaving before the grant receipt. Screenshots show readable recovery; the retry control measured 153 × 48 points. The full portable package passed 270 tests. Simulator evidence does not establish physical haptic sensation, minimum watchOS, real model/tool execution or distribution.

The standalone target requires `WKWatchOnly: true` without `WKRunsIndependentlyOfCompanionApp`. Both the generated specification and checked-in plist must agree. Compilation missed the original install errors; verify the built artifact and actually install it. Preserve all earlier failed attempts when correcting packaging.

Watch text fields launch a separate system `TextView` editor. Locate the focused editor by its observed placeholder, enter text there, finish with Done, and verify the field value. Derive scroll bounds from the largest-area window, since the scroll indicator may be designated the main window; use bounded small drags toward the target. Fleet navigation rows expose buttons, not static text. Loop 49 retains failed locator/editor attempts separately from the final successful batch. Never inject a token to turn those failures into passing pairing evidence.

All local evidence is under `.omb-scratch/verification/loop49-watch/`. A new run needs fresh owned identities, frozen source and sequential completed build/test steps. Cleanup requires the exact native wrapper, xcodebuild and Watch test runner to be terminal, a valid process observation and the owned CA hash; never reuse the prior simulator after cleanup. Watch reply-draft reliability remains separate: its legacy chat composer is not covered by these approval tests.
