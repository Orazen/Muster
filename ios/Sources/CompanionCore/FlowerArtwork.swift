// The flower itself: the authored body path, its eye anchors, and the agent
// palette — the parts every client must agree on.
//
// This used to live inside the phone's `FlowerAvatar.swift`, which meant the
// watch could only have the same face by copying the path a second time. One
// copy, in the core, with tests. The core stays Foundation-only (the README's
// rule for this module), so the path is published as plain numbers rather
// than any drawing type: each view builds its own `Path`.
import Foundation

/// One instruction from the SVG path grammar the artwork uses. Only `M`, `C`
/// and `Z` appear in the body, so those are the only cases.
public enum FlowerPathCommand: Equatable, Sendable {
    case move(x: Double, y: Double)
    case curve(c1x: Double, c1y: Double, c2x: Double, c2y: Double, x: Double, y: Double)
    case close
}

public enum FlowerArtwork {
    /// MusterMascot's MUSTER_BODY — the artwork emitted to /app-icon.svg.
    /// Verbatim; only `M`, `C` and `Z` appear, which the parser below relies
    /// on.
    public static let bodyPath =
        """
    ...
    /// Eye anchors in viewBox units, left then right. The authored -4° tilt
    /// is applied by each view at the anchor.
    public static let eyes: [(x: Double, y: Double)] = [(-15, -2), (38, -6)]

    /// The parsed body, built once at first use.
    public static let body: [FlowerPathCommand] = parse(bodyPath)

    /// src/lib/mascot.ts — AGENT_COLORS. A bot the owner knows as "the orange
    /// one" must be the same orange on every screen.
    public static let agentHex: [String: String] = [
        "green": "#009957",
        "blue": "#377FE6",
        "red": "#D94B52",
        "orange": "#E78531",
    ...
    /// The flower's own default, which is not the neutral grey the generic
    /// palette accessor falls back to: an unnamed agent still looks like a
    /// Muster bot.
    public static let flowerFallbackHex = "#f08a24"

    public static func agentColorHex(_ name: String) -> String {
        agentHex[name] ?? flowerFallbackHex
    }

    /// SVG path → commands. Handles the subset the artwork uses: `M` (first
    /// pair only), `C` (many six-number curves after one letter), `Z`, and
    /// whitespace/comma separators with signed and exponent numbers.
    static func parse(_ d: String) -> [FlowerPathCommand] {
        var commands: [FlowerPathCommand] = []
        var numbers: [Double] = []
        ...
        func flush() {
            ...
            case "M":
                if numbers.count >= 2 {
                    commands.append(.move(x: numbers[0], y: numbers[1]))
                }
            case "C":
                var index = 0
        ...
        for character in d {
            if character.isNumber || character == "." || character == "e" {
                ...
                takeNumber(); flush(); commands.append(.close); command = nil
            } else {
