...
function statusState(status: RoutineRunStatus): AgentState {
  switch (status) {
    case "queued":
      return "drowsy";
    case "running":
...
export function RoutineEditor({
  ...
  const [runOn, setRunOn] = useState<RoutineRunOn>(routine?.runOn ?? defaultRunOn ?? "agent");
      ...
      <div className="max-h-[90vh] w-full max-w-[620px] overflow-y-auto rounded-2xl border border-hairline/60 bg-panel shadow-2xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-hairline/40 bg-panel/95 px-5 py-4 backdrop-blur">
          <div>
            ...
            <div className="mt-0.5 text-[12px] text-ink-secondary">Give a teammate scheduled work with a calendar you can trust.</div>
          </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              <button
                ...
                className={cn(
                  ...
                  runOn === "agent" ? "border-accent/70 bg-accent/10" : "border-hairline/50 bg-inset hover:bg-raised/60",
                )}
              >
                <div className="flex items-center gap-2 text-[13px] font-medium text-ink"><Laptop size={15} />This computer</div>
                <div className="mt-1 text-[11px] leading-relaxed text-ink-secondary">Uses this teammate's selected model and computer setting.</div>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <div className="mt-1 text-[11px] leading-relaxed text-ink-secondary">Runs the teammate and its tools inside its Box virtual machine.</div>
              </button>
              <button
                type="button"
        ...
        <div className="max-h-[55vh] space-y-4 overflow-y-auto p-5">
          ...
          {run?.triggerSource === "webhook" && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-xl bg-inset p-3"><div className="text-[10px] uppercase tracking-wider text-ink-secondary">Runs on</div><div className="mt-1 flex items-center gap-1.5 text-[13px] text-ink">{run.runOn === "cloud" || run.runOn === "opensandbox" ? <Cloud size={13} /> : <Laptop size={13} />}{run.runOn === "cloud" ? "Cloud VM" : run.runOn === "opensandbox" ? "OpenSandbox" : "Teammate setup"}</div></div>
          ...
          {run?.status === "waiting" && <div className="rounded-xl border border-warning/30 bg-warning/10 px-3.5 py-3 text-[13px] text-warning">This teammate needs your answer. Open its task to continue the run.</div>}
        </div>
          ...
          {routines.map((routine) => {
            ...
            return (
              <div key={routine.id} className="flex items-center gap-3 rounded-xl border border-hairline/40 bg-inset p-3">
                ...
                <div className="min-w-0 flex-1"><div className="truncate text-[14px] font-semibold text-ink">{routine.name}</div><div className="mt-0.5 truncate text-[11.5px] text-ink-secondary">{bot?.name ?? "Deleted teammate"} · {scheduleLabel(routine)}</div></div>
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[12.5px] text-ink-secondary">{section === "calendar" ? "Run teammate tasks on a schedule." : "Run teammate tasks when an event arrives."}</p>
          </div>
      ...
      {section === "webhooks" ? (
        ...
        <div className="flex min-h-0 flex-1 items-center justify-center p-8">
          <div className="max-w-[430px] text-center">
            ...
            <h2 className="text-[18px] font-semibold text-ink">Put your team on a rhythm</h2>
