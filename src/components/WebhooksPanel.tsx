...
/** A destination-first webhook view: choose a AGENT endpoint on the left, then
      ...
      <div className="mx-auto max-w-[1120px] space-y-4">
        <header className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[12px] text-ink-secondary">Send a task to a AGENT when another tool reports an event.</p>
          </div>
        ...
        {state.webhooks.length === 0 ? (
          <div className="border-t border-hairline/40 px-6 py-16 text-center">
            ...
            <p className="mx-auto mt-2 max-w-[420px] text-[12.5px] leading-relaxed text-ink-secondary">Choose a AGENT, copy one command, and every request becomes a new task in its chat.</p>
            ...
            {bots.length === 0 && <p className="mt-3 text-[12px] text-warning">Create a AGENT first, then come back here.</p>}
          </div>
        ) : selected && (
