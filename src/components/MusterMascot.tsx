// One authored flower geometry for Muster branding — the body constant stays
// exact: scripts/make-brand-icons.mjs and src/tray-main.ts read these exports
// verbatim (the script's regex `export const NAME = "..."` must keep matching).
import { AgentBotAvatar } from "./AgentBotAvatar";

export const MUSTER_ORANGE = "#f08a24";
export const MUSTER_EYES = "#f9f9f9";
// SAFETY: scripts/make-brand-icons.mjs matches `export const NAME = "…"` on a
// single line — keep this assignment unwrapped or brand icons stop building.
...
interface MusterMascotProps {
  ...
  color?: string;
  eyes?: MusterEyeState;
  /** Static expression, independent of optional blinking (bot-avatars blinks its own). */
  eyeOpenness?: number;
  label?: string;
  animated?: boolean;
...
/** Muster's flower mascot mark, drawn by `bot-avatars` through the shared adapter. */
export function MusterMascot({
  ...
  return (
    <AgentBotAvatar
      ...
      state={eyes === "closed" ? "sleeping" : undefined}
      label={label}
      animated={animated}
      // `reaction` keys a re-mount so one finite wave replays without a timer.
      seed={`muster-mascot:${reaction}`}
    />
  );
}
