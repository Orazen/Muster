...
interface ScenarioLabels extends Partial<Record<AgentState, string>> {}
...
} satisfies Record<Exclude<AgentMotion, "none">, string>;

const MOTION_COLORS: AgentColor[] = [
...
function MotionCard({
  ...
  motion: Exclude<AgentMotion, "none">;
  index: number;
  replayAll: number;
  color: AgentColor;
}) {
  ...
  return (
    <main className="preview-shell">
      ...
      <section className="motion-grid" aria-label="Mascot motion library">
        {AGENT_MOTIONS.map((motion, index) => (
          <MotionCard
            key={motion}
            motion={motion}
            index={index}
      ...
      <section className="expression-library" aria-labelledby="expression-heading">
        <div className="section-heading">
          ...
          <p>Move your pointer over any Agent to test the responsive eyes.</p>
        </div>
            ...
            {AGENT_COLOR_NAMES.map((c) => (
              <div className="matrix-row" key={c}>
                <div className="row-label">
                  <span className="swatch" style={{ background: AGENT_COLORS[c] }} />
                  <strong>{c}</strong>
                  <code>{AGENT_COLORS[c]}</code>
                </div>
                {PICKABLE_STATES.map((s) => (
                  <div className="mascot-cell" key={`${c}-${s}`}>
                    <AgentAvatar color={c} state={s} size={86} label={`${c} ${s} agent`} />
                  </div>
                ))}
              </div>
            ))}
