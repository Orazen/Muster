/**
 * Musterbot logo mark — the canonical brand mark.
 *
 * Exactly the artwork served at /app-icon.svg: the authored five-lobed
 * flower body with two off-white capsule eyes, nothing added (no smile —
 * the brand mark is the app icon). The old generated-blob geometry is gone;
 * git history keeps it. `eyes` keeps its historical variants so existing
 * callers compile: "open" is the canonical rest state, "closed"/"happy"
 * draw the matching arc shapes in the same eye slots.
 */
export const MUSTERBOT_ORANGE = "#f08a24";

/** MusterMascot's MUSTER_BODY — the authored five-lobed flower path. */
const MUSTER_BODY =
...
/** The app-icon's eye anchors: each group translates, tilts -4°, and holds
 * one portrait capsule (21×44, rx 10.5) centred on its own origin. */
const EYES = [
  { x: -15, y: -2 },
  { x: 38, y: -6 },
] as const;
...
export function MusterBotMark({
  ...
  return (
    ...
    >
      ...
      {/* the app icon draws the body flat; the gradient stays for callers
          that pass a custom color and still reads as the same mark */}
      {color === MUSTERBOT_ORANGE ? (
        <path d={MUSTER_BODY} fill={color} />
      ) : (
        <path d={MUSTER_BODY} fill={`url(#${gid})`} />
      )}
      {EYES.map((eye) => (
        <g key={eye.x} transform={`translate(${eye.x} ${eye.y}) rotate(-4)`}>
          {eyes === "open" ? (
            <rect x={-10.5} y={-22} width={21} height={44} rx={10.5} fill="#f9f9f9" />
          ) : (
            <path
              d={eyes === "happy" ? "M -9 4 Q 0 -6 9 4" : "M -9 -2 Q 0 8 9 -2"}
              stroke="#f9f9f9"
              strokeWidth={7}
              strokeLinecap="round"
              fill="none"
