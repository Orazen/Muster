...
const service: ServiceInfo = {
  ...
  type: "_muster._tcp",
  port: 8800,
  host: "muster-1a2b3c4d.local",
  addresses: ["192.168.1.42"],
  txt: ["v=1", "name=Alex's computer"],
};

const INSTANCE = "Alex's computer._muster._tcp.local";
const SERVICE_NAME = "_muster._tcp.local";
...
describe("naming", () => {
  it("keeps an instance name to a single valid label", () => {
    ...
    expect(dnsLabel("")).toBe("Muster");
    expect(dnsLabel("   ")).toBe("Muster");
  });

  it("claims a host name the system responder will not fight us for", () => {
    const name = defaultHostName("Alexs-MacBook-Pro");
    expect(name).toMatch(/^muster-[0-9a-f]{8}\.local$/);
    // stable across restarts, distinct per machine
    expect(defaultHostName("Alexs-MacBook-Pro")).toBe(name);
    expect(defaultHostName("another-machine")).not.toBe(name);
    expect(name).not.toContain("Alexs-MacBook-Pro");
  });
