...
Muster's harness listens on `127.0.0.1` and nothing else, which is the
...
| `OMB_COMPANION_DIR` | `~/.muster-companion` | paired devices live here |
...
back to `Muster` when the harness is not up or has no profile. Read once
